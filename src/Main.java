...
public class Main {
    ...
    public static void main(String[] args) {
       ...
       try {
           ...
           con = DriverManager.getConnection(connectionUrl, "postgres", "0311");
           ...
           rs = stmt.executeQuery("select * from users_clients");

           while (rs.next()) // Processing the result
               System.out.println(rs.getInt("user_client_id") + "  "
                       + rs.getString("name") + "  " + rs.getString("surname") + "  "
                       + rs.getString("male") + "  " + rs.getString("age") + "  "
